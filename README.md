...
Multiple processes that run in a computer may need to access the same resources at the same time. Take a file for example. Some processes would **write** to that file and others would **read** from that file. The contents of the file are updated while it is being written to, but not when it is being read from. Hence, it is okay for two processes to simultaneously read from it, but not for any other process to access a resource when a process is writing to it.
